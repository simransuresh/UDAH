...
from helpers import signal, noise, dist_mat, dist_pt, PV, covar1, covar2, tdiff
import math
import random
import csv
...
with open('results/dh_2011_2018.csv', mode='r') as file:
        ...
        for row in reader:
                ...
                hydr_data[(float(row['Latitude']), float(row['Longitude']))] = {
                        'dt': row['Datetime'], 
                        'ssh': float(row['Dynamic_height']) if row['Dynamic_height']!='' else np.nan
                }
